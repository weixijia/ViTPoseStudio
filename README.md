...
ViTPose Studio is a completely overhauled, modern Mac-native desktop application for high-performance, real-time skeleton pose estimation. It extracts the foundational capabilities of the ViTPose engine and pairs them with a beautiful graphical user interface built with `customtkinter`.

## Features
- **Modern Mac-Native UI**: Sleek, fully-responsive dark-mode interface with a side control panel and live camera preview.
...
- **Synchronized AV Recording Engine**: Uses a robust FFmpeg integration to perfectly synchronize live audio (from your Mac's microphone) with the skeleton-overlaid video. Handles hardware frame-rate jitter effortlessly by zero-padding to a strict 30 FPS.
