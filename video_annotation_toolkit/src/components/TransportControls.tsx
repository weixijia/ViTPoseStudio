  ...
  return (
    <div className="transport">
      <div className="transport-group">
        <button title="First frame (Home)" onClick={() => goto(0)}>⏮</button>
        <button className="frame-btn" title="Back 10 frames (Shift+←)" onClick={() => step(-10)}>−10f</button>
        <button className="frame-btn" title="Previous frame (←)" onClick={() => step(-1)}>◀ 1f</button>
        <button className="play" title="Play / Pause (Space)" onClick={togglePlay}>
          {isPlaying ? '❙❙' : '▶'}
        </button>
        <button className="frame-btn" title="Next frame (→)" onClick={() => step(1)}>1f ▶</button>
        <button className="frame-btn" title="Forward 10 frames (Shift+→)" onClick={() => step(10)}>+10f</button>
        <button title="Last frame (End)" onClick={() => goto(meta.frameCount - 1)}>⏭</button>
      </div>
