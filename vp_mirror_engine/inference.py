...
class VitInference:
    ...
    Args:
        ...
        dataset (str, optional): Name of the dataset. If None it's extracted from the file name.
                                 Valid values are 'coco', 'coco_25', 'wholebody', 'mpii',
                                                  'ap10k', 'apt36k', 'aic'
        yolo_size (int, optional): Size of the input image for YOLO model. Defaults to 320.
    ...
    def __init__(self, model: str,
        ...
        assert os.path.isfile(yolo), f'The YOLO model {yolo} does not exist'
    ...
    def inference(self, img: np.ndarray) -> dict[typing.Any, typing.Any]:
        ...
        for bbox, id, score in zip(bboxes, ids, scores):
            ...
            if bbox[3] <= bbox[1] or bbox[2] <= bbox[0]:
                # Skip this detection as it has invalid dimensions
                continue
    ...
    def draw(self, show_yolo=True, show_raw_yolo=False, confidence_threshold=0.5, mono_color_mode=False, skeleton_thickness=2):
        """
        ...
        Args:
            ...
            mono_color_mode (bool, optional): Whether to use mono color mode. Default is False.
            skeleton_thickness (int, optional): Thickness of skeleton lines. Default is 2.
        ...
        for idx, k in self._keypoints.items():
            if mono_color_mode:
                # Use monochrome mode - BGR format of #9bc472
                mono_color = (114, 196, 155)
                img = draw_points_and_skeleton(img.copy(), k,
                                              joints_dict()[self.dataset]['skeleton'],
                                              person_index=idx,
                                              points_color_palette=None,
                                              skeleton_color_palette=None,
